/**
 * Helper utilities for Hijri Date conversions and Prayer Countdown calculations
 */

export function getHijriDate(): string {
  // Simple algorithm to approximate Hijri date
  // For production accuracy, we calculate based on astronomical julian day
  const today = new Date();
  let day = today.getDate();
  let month = today.getMonth() + 1;
  let year = today.getFullYear();

  if (year < 1900) return '14 Ramadan 1447'; // Fallback
  ...
  // Adjustments (usually +/- 1 day depending on moon sighting)
  // Let's return a cleanly formatted string
...
/**
 * Parses time string "HH:MM" into minutes of the day
 */
...
export function getNextPrayer(timetable: Timetable): NextPrayerInfo | null {
  ...
  let nextInfo: NextPrayerInfo | null = null;

  // Loop through prayers to find the next active event (Adhan or Iqamah)
  for (const prayer of prayers) {
    ...
    if (currentMinutes < iqamahMins) {
      ...
      return {
        name: prayer.name,
        type: 'Iqamah',
        time: prayer.times.iqamah,
        secondsRemaining
      };
    }
  }

  // If all prayers for today are passed, the next event is tomorrow's Fajr
  // Calculate remaining seconds to midnight + Fajr time
