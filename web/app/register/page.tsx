...
export default function RegisterPage() {
  ...
  const [role, setRole] = useState('mosque_admin'); // Defaults to Mosque Admin for new signups
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-900 text-slate-100 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-950/80 border border-slate-800 rounded-3xl p-6 md:p-8 space-y-8 relative z-10 backdrop-blur-xl shadow-2xl shadow-black/40">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-xl font-bold tracking-tight text-white mt-4">Create Admin Account</h2>
          <p className="text-xs text-slate-400">Register as a Mosque Admin to manage schedules</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          <div className="space-y-4">
            ...
            {/* Role Selection (locked to Mosque Admin or Community User for general register, Super Admins created manually or first-run) */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-400">Signup Role</label>
              <select
                value={role}
                onChange={(e) => setRole(e.target.value)}
                className="w-full px-4 py-2.5 border border-slate-800 focus:border-primary rounded-xl bg-slate-900/50 text-sm outline-none text-white transition-colors"
              >
                <option value="mosque_admin">Mosque Administrator (Murobi/Imam)</option>
                <option value="community_user">General Community Member</option>
              </select>
            </div>
          </div>
